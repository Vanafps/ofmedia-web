    ...
    <section className="space-y-2.5 py-2">
      ...
      <div className="flex items-end justify-between px-4 sm:px-6 lg:px-8 max-w-[1440px] mx-auto">
        ...
        <div className="hidden sm:flex items-center gap-1.5 text-[11px] text-zinc-400 font-normal select-none px-3 py-1 rounded-full bg-white/[0.04] backdrop-blur-xl border border-white/10">
          ...
          <span>Перетягивайте ленту мышью</span>
        </div>
      </div>
      ...
      <div
        ...
        className={`flex items-start gap-4 sm:gap-5 overflow-x-auto px-4 sm:px-6 lg:px-8 max-w-[1440px] mx-auto pb-4 pt-1 select-none transition-cursor duration-150 ${
        ...
        {projects.map((project) => {
          ...
          return (
            <div
              ...
              className="flex-none w-[260px] sm:w-[300px] group cursor-pointer space-y-2 relative"
            >
              {/* Card Thumbnail Box with Glass Hover Glow */}
              <div className="relative aspect-video rounded-2xl overflow-hidden bg-[#101014] border border-white/10 group-hover:border-[#ff5c00]/50 transition-all duration-300 shadow-md group-hover:shadow-[0_12px_32px_rgba(255,92,0,0.22)] group-hover:-translate-y-1">
                <img
                  src={project.poster}
                  alt={project.title}
                  draggable={false}
                ...
                {/* Bottom Badges with Frosted Glass & Centered PlayIcon */}
                <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-center justify-between pointer-events-none">
                  <button
                    ...
                    className={`pointer-events-auto p-2 rounded-xl backdrop-blur-2xl transition-all shadow-lg border ${
                      isFav
                        ? 'bg-[#ff5c00] text-white border-[#ff5c00] shadow-[0_0_12px_rgba(255,92,0,0.5)]'
                        : 'bg-black/60 hover:bg-black/80 text-zinc-200 hover:text-white border-white/20 hover:scale-105'
                    }`}
                  ...
                  <button
                    ...
                    className="pointer-events-auto flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#ff5c00] hover:bg-[#e05200] text-white text-[11px] font-medium shadow-lg hover:scale-105 active:scale-95 transition-all border border-white/20"
                  >
              ...
              <div className="space-y-0.5 px-0.5">
                <h3 className="font-heading font-medium text-sm text-white group-hover:text-[#ff5c00] transition-colors truncate">
                  {project.title}
                </h3>
